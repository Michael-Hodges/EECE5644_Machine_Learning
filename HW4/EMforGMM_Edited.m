function [logLikelihood, alpha,mu,Sigma] =  EMforGMM_Edited(N,d, data, parameters)
...
regWeight = 5e-3; % regularization parameter for covariance estimates
...
d = d; % Dimension of data coming in.
M = parameters;
x = data;
...
while ~Converged
    for l = 1:M % multiply prior with PDF evaluated at certain point
        temp(l,:) = repmat(alpha(l),1,N).*evalGaussian(x,mu(:,l),Sigma(:,:,l));
    end
    plgivenx = temp./sum(temp,1); % Probability of each given data
    alphaNew = mean(plgivenx,2); % New Priors
    w = plgivenx./repmat(sum(plgivenx,2),1,N); % Probability of given data normalized
    muNew = x*w'; % update means mutliplying data by prior distributions
    for l = 1:M
        v = x-repmat(muNew(:,l),1,N);
        u = repmat(w(l,:),d,1).*v;
        SigmaNew(:,:,l) = u*v' + regWeight*eye(d,d); % adding a small regularization term
    end
    Dalpha = sum(sum(abs(alphaNew-alpha)));
